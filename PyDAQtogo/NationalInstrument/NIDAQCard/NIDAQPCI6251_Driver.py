# -*- coding: utf-8 -*-
"""
Created on Wed Feb 23 00:35:27 2022
...
class StepSinGenerator(Task):
    ...
    def __init__(self, m_freq, m_ampl, v_min, v_max, v_scanRate, CV=0, reverse= 1, step= 0.002, offset = 0 ):
        ...
        period = (v_max-v_min)/v_scanRate*2
        ...
        if freq >1000:
            self.sampleRate =  50.0*freq # Samples/second, maxmium samplingrate is 250K per channel
            self.maxNsamples = 50.0*freq
        else:
        ...
        self.squ = (v_max-v_min)*1e-3*signal.square(2*np.pi *period*self.t, duty=0.5)
        # Setup DAQ:
        read = int32()
...
#        self.AC_ref = np.append(self.ACStep,self.sin/max(self.sin)*0.5, axis=0) #self.sin/max(self.sin)*1.0 is the reference signal to lockin
        ...
        # self.AC_ref = np.append(self.ACStep*reverse,self.sin, axis=0)
        self.CreateAOVoltageChan("Dev2/ao0", "", -0.5, 0.5, DAQmx_Val_Volts, None)
        self.CreateAOVoltageChan("Dev2/ao1", "", -0.5, 0.5, DAQmx_Val_Volts, None)
...
class MeasureTask(Task):
    def __init__(self, freq, rt):
        ...
        if freq > 400:
            ...
            if freq>800:
                ...
                if freq>1500:
                    self.samplerate = 50.0*freq
        else:
            self.samplerate = 100.0*freq 
        self.updatadataevery = 100
        self.inputchannelsN = 2
        # if freq< 600:
        #     self.inputchannelsN = 6
        self.data = np.zeros(self.inputchannelsN*self.updatadataevery)
        self.a = []
        self.CreateAIVoltageChan("Dev2/ai0","Voltage",DAQmx_Val_Diff,-1, 1,DAQmx_Val_Volts,None)# from Edaq waveform
        self.CreateAIVoltageChan("Dev2/ai1","Current",DAQmx_Val_Diff,-1, 1,DAQmx_Val_Volts,None)# from eDaq currents
        # self.CreateAIVoltageChan("Dev2/ai3","LockinX",DAQmx_Val_Diff,-10.0,10.0,DAQmx_Val_Volts,None) #from lockin output x
        # self.CreateAIVoltageChan("Dev2/ai4","PD",DAQmx_Val_Diff,-10.0,10.0,DAQmx_Val_Volts,None) #from femto PD
        # self.CreateAIVoltageChan("Dev2/ai5","LockinY",DAQmx_Val_Diff,-10.0,10.0,DAQmx_Val_Volts,None) #from lockin output y
        # if freq< 600:
        #     self.inputchannelsN = 6
        ...
        # self.CfgInputBuffer(int(self.samplerate *(rt*10)))
        self.CfgSampClkTiming("",self.samplerate,DAQmx_Val_Rising,DAQmx_Val_ContSamps,self.inputchannelsN*self.updatadataevery) # 1000.0 is the sampling rate
    ...
    def EveryNCallback(self):
        ...
        # self.ReadAnalogF64(self.updatadataevery, 10.0, DAQmx_Val_GroupByScanNumber, self.data, self.updatadataevery, byref(read), None)
...
class ZeroOutput(Task):
    def __init__(self):
        Task.__init__(self)
        self.CreateAOVoltageChan("Dev2/ao0","",-10.0,10.0,DAQmx_Val_Volts,None)
        self.CreateAOVoltageChan("Dev2/ao1","",-10.0,10.0,DAQmx_Val_Volts,None)
